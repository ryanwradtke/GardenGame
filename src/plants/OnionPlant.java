 ...
 * @author Ryan
 */
public class OnionPlant extends Plant {

    List<ProduceInterface> produce = new ArrayList();
    
    private final String type = "Onion";
    
    private double size = 0; // 0 - 10: 10 is largest for variety of plant.
    private double health = 2; // 0 - 10, 10 highest
    
    private final int plantsPerSpot = 6; //Plants per Garden Spot-e.g. onions will have 6.
    private final double produceLimit = 0; //% of size or 0 if only allowed 1 per plant.
    private final double growthSpeed = .75; //growth per turn. (10 turns per season.)
   
    private final int sunlteReq = 1; // 1- 5: 5 is highest need for sunlight
    private final int waterReq = 2; // 1-5: 5 is highest need for water
    
    private final double hardiness = .9; // % resistance to weather, also health increase.
